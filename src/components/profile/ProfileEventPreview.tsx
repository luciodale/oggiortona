        ...
        <div>
          <div className="flex flex-wrap gap-1">
            {categories.map((cat) => (
              <span
                key={cat}
                className={`inline-block rounded-full px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${eventCategoryColors[cat] ?? "bg-stone-100 text-stone-600"}`}
              >
                {catLabels[cat] ?? cat}
              </span>
            ))}
          </div>
          <h1 className="mt-2 font-family-display text-2xl font-medium leading-tight text-primary">
            {event.title}
          </h1>
        </div>
      </div>
