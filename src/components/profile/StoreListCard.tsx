...
export function StoreListCard({ store }: StoreListCardProps) {
  const { t } = useLocale();
  const { handleDelete } = useDeleteEntity("store");
  const { openStoreForm, openStoreStorefront, openStorePromotionsList } = useFormSheet();

  const hasPromotions = store.promotions.length > 0 || store.expiredPromotionCount > 0;
  const isPending = store.active === 0;
  const hasExpired = store.expiredPromotionCount > 0;
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          ...
          {hasExpired && (
            <span className="inline-block rounded-full bg-danger/10 px-2.5 py-0.5 text-[10px] font-semibold text-danger">
              {store.expiredPromotionCount} {store.expiredPromotionCount === 1 ? t("promo.expiredOne") : t("promo.expiredMany")}
            </span>
          )}
        </div>
      )}

      <p className="mt-3 mb-1.5 text-[10px] font-semibold uppercase tracking-[0.15em] text-muted">{t("stores.promotions")}</p>
      <div className="flex gap-2">
        <button
          type="button"
          onClick={() => openStoreStorefront(store.id)}
        ...
        {hasPromotions && (
          <button
            type="button"
            onClick={() => openStorePromotionsList(store.id)}
